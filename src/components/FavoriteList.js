        ...
        <div className="relative">
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white shadow-xl rounded-lg border p-4 z-50">
                    <h4 className="font-bold border-bottom pb-2 mb-2 text-black">
                        Favorites:
                    </h4>
                    {favs.length===0 ? (
                        ...
                        <ul className="flex flex-col gap-2">
                            ...
                            fav && fav.id && self.findIndex(f=>f.id===fav.id)===index).map(fav=>(
                                <li key={fav.id} className="border-b last:border-b-0 pb-1 border-gray-100">
                                    <Link className="text-[#AE445A] hover:underline text-sm"
